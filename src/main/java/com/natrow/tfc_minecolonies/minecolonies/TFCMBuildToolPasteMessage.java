...
import com.natrow.tfc_minecolonies.util.TFCMStructurePlacement;
...
/**
 * Send build tool data to the server. Verify the data on the server side and then place the
 * building. This has been extended to include TFCM specific data about material types.
 */
public class TFCMBuildToolPasteMessage implements IMessage {
  ...
  private static void handleHut(
    ...
    final ItemStack stack =
        BuildingUtils.getItemStackForHutFromInventory(player.getInventory(), hut);
    final Block block =
        stack.getItem() instanceof BlockItem ? ((BlockItem) stack.getItem()).getBlock() : null;
    if (block != null && EventHandler.onBlockHutPlaced(world, player, block, buildPos)) {
      world.destroyBlock(buildPos, true);
      world.setBlockAndUpdate(buildPos, state);
      if (!complete) {
        ((IAbstractBlockHutExtension) block)
            .onBlockPlacedByBuildTool(
                world,
                buildPos,
                world.getBlockState(buildPos),
                player,
                null,
                mirror,
                sn.getStyle(),
    ...
    if (building == null) {
      ...
      if (building.getTileEntity() != null) {
        final IColony colony =
            IColonyManager.getInstance().getColonyByPosFromWorld(world, buildPos);
        if (colony == null) {
  ...
  public void onExecute(final NetworkEvent.Context ctxIn, final boolean isLogicalServer) {
    ...
    if (!Structures.hasMD5(sn)) {
      MessageUtils.format(
              new TextComponent("Can not build " + workOrderName + ": schematic missing!"))
          .sendTo(player);
      return;
    }
    ...
    if (player.isCreative()) {
      if (isHut) {
        handleHut(
            CompatibilityUtils.getWorldFromEntity(player),
            player,
            sn,
            rotation,
            pos,
            ...
            rockType,
            soilType);
        TFCMStructurePlacement.loadAndPlaceBuildingWithRotation(
            player.level,
            structureName,
            pos,
        ...
        final IBuilding building =
            IColonyManager.getInstance()
                .getBuilding(CompatibilityUtils.getWorldFromEntity(player), pos);
        if (building != null) {
          final WorkOrderBuilding workOrder =
              WorkOrderBuilding.create(WorkOrderType.BUILD, building);
          ConstructionTapeHelper.removeConstructionTape(
              workOrder, CompatibilityUtils.getWorldFromEntity(player));
        }
      } else {
        TFCMStructurePlacement.loadAndPlaceStructureWithRotation(
            ctxIn.getSender().level,
            structureName,
            pos,
            BlockPosUtil.getRotationFromRotations(rotation),
            mirror ? Mirror.FRONT_BACK : Mirror.NONE,
      ...
      if (player.getStats().getValue(Stats.ITEM_USED.get(ModItems.supplyChest)) > 0
          && !MineColonies.getConfig().getServer().allowInfiniteSupplyChests.get()
            ...
            searchPredicate.and(
                stack ->
                    ItemStackUtils.compareItemStacksIgnoreStackSize(
                        stack, new ItemStack(ModItems.supplyChest), true, false));
      }
      if (structureName.contains("supplycamp")) {
        searchPredicate =
            searchPredicate.and(
                stack ->
                    ItemStackUtils.compareItemStacksIgnoreStackSize(
                        stack, new ItemStack(ModItems.supplyCamp), true, false));
      }
  ...
  private boolean isFreeInstantPlacementMH(ServerPlayer playerEntity) {
    ...
    return !ItemStackUtils.isEmpty(mhItem)
        && mhItem.getTag() != null
