 ...
 * building. Expanded to include TFCM-specific information about material types.
 */
  ...
  private static void handleHut(
    ...
    final int slot =
        InventoryUtils.findFirstSlotInItemHandlerWith(
            ...
            s -> ItemStackUtils.compareItemStacksIgnoreStackSize(tempStack, s, false, false));
    if (slot < 0) {
      return;
    }
    ...
    if (tempColony != null
        && compound != null
        && compound.contains(TAG_COLONY_ID)
        && tempColony.getID() != compound.getInt(TAG_COLONY_ID)) {
      ...
      if (EventHandler.onBlockHutPlaced(world, player, block, buildPos)) {
        if (tempColony != null) {
          AdvancementUtils.TriggerAdvancementPlayersForColony(
              tempColony, playerMP -> AdvancementTriggers.PLACE_STRUCTURE.trigger(playerMP, sn));
        } else {
