...
public final class TFCMClientEventHandler
{
    ...
    {
        event.enqueueWork(() -> {
            Stream.of(Wood.BlockType.SAPLING, Wood.BlockType.DOOR, Wood.BlockType.TRAPDOOR, Wood.BlockType.FENCE, Wood.BlockType.FENCE_GATE, Wood.BlockType.BUTTON, Wood.BlockType.PRESSURE_PLATE, Wood.BlockType.SLAB, Wood.BlockType.STAIRS, Wood.BlockType.TWIG, Wood.BlockType.BARREL, Wood.BlockType.SCRIBING_TABLE, Wood.BlockType.POTTED_SAPLING)
                .forEach(type -> ItemBlockRenderTypes.setRenderLayer(TFCMBlocks.PLACEHOLDER_WOODS.get(type)
                    .get(), RenderType.cutout()));
            Stream.of(Wood.BlockType.LEAVES, Wood.BlockType.FALLEN_LEAVES)
                .forEach(type -> ItemBlockRenderTypes.setRenderLayer(TFCMBlocks.PLACEHOLDER_WOODS.get(type)
                    .get(), layer -> Minecraft.useFancyGraphics() ? layer == RenderType.cutoutMipped() : layer == RenderType.solid()));
            Stream.of(SoilBlockType.GRASS, SoilBlockType.CLAY_GRASS)
                .forEach(type -> ItemBlockRenderTypes.setRenderLayer(TFCMBlocks.PLACEHOLDER_SOIL.get(type)
    ...
    public static void colorHandlerBlocks(ColorHandlerEvent.Block event)
    {
        ...
        registry.register(foliageColor, TFCMBlocks.PLACEHOLDER_WOODS.get(Wood.BlockType.LEAVES)
            .get(), TFCMBlocks.PLACEHOLDER_WOODS.get(Wood.BlockType.FALLEN_LEAVES)
            .get());

        registry.register(grassColor, TFCMBlocks.PLACEHOLDER_SOIL.get(SoilBlockType.GRASS)
            .get(), TFCMBlocks.PLACEHOLDER_SOIL.get(SoilBlockType.CLAY_GRASS)
            .get());
    }
    ...
    public static void colorHandlerItems(ColorHandlerEvent.Item event)
    {
        ...
        registry.register(foliageColor, TFCMBlocks.PLACEHOLDER_WOODS.get(Wood.BlockType.FALLEN_LEAVES)
            .get(), TFCMBlocks.PLACEHOLDER_WOODS.get(Wood.BlockType.LEAVES)
            .get());
    }

    public static void onTextureSwitch(TextureStitchEvent.Pre event)
    {
        final ResourceLocation sheet = event.getAtlas()
            .location();
