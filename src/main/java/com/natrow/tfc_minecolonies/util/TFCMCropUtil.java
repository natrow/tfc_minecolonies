...
public class TFCMCropUtil {
  ...
  public static List<Item> getFertilizers(CropBlock crop, @Nullable FarmlandBlockEntity farmland) {
    ...
    return Fertilizer.MANAGER.getValues().stream()
        .filter(
            fertilizer -> {
        ...
        .sorted(
            (item1, item2) ->
                Float.compare(getNutrient(item1, nutrient), getNutrient(item2, nutrient)))
        .toList();
  }
  ...
  public static float getNutrient(
      Fertilizer fertilizer, FarmlandBlockEntity.NutrientType nutrient) {
