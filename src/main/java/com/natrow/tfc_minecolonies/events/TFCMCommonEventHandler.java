...
public class TFCMCommonEventHandler {
    ...
    try {
      if (event.getPackType() == PackType.CLIENT_RESOURCES) {
        ...
        PathResourcePack pack =
            new PathResourcePack(modFile.getFileName() + ":overload", filePath) {
              @Nonnull
              @Override
          ...
          event.addRepositorySource(
              (consumer, constructor) ->
                  consumer.accept(
                      constructor.create(
                          "builtin/tfcm_data",
                          new TextComponent("TFCM Resources"),
                          true,
                          () -> pack,
                          metadata,
                          Pack.Position.TOP,
  ...
  private static void onCommonSetup(FMLCommonSetupEvent event) {
    extendBlockEntityType(
        TFCBlockEntities.CHEST.get(),
        TFCMBlocks.PLACEHOLDER_WOODS.get(Wood.BlockType.CHEST).stream());
    extendBlockEntityType(
        TFCBlockEntities.TRAPPED_CHEST.get(),
        TFCMBlocks.PLACEHOLDER_WOODS.get(Wood.BlockType.TRAPPED_CHEST).stream());
    extendBlockEntityType(
        TFCBlockEntities.BARREL.get(),
        TFCMBlocks.PLACEHOLDER_WOODS.get(Wood.BlockType.BARREL).stream());
    extendBlockEntityType(
        TFCBlockEntities.LOOM.get(),
        TFCMBlocks.PLACEHOLDER_WOODS.get(Wood.BlockType.LOOM).stream());
    extendBlockEntityType(
        TFCBlockEntities.SLUICE.get(),
        TFCMBlocks.PLACEHOLDER_WOODS.get(Wood.BlockType.SLUICE).stream());
    extendBlockEntityType(
        TFCBlockEntities.TOOL_RACK.get(),
        TFCMBlocks.PLACEHOLDER_WOODS.get(Wood.BlockType.TOOL_RACK).stream());
    extendBlockEntityType(
        TFCBlockEntities.BOOKSHELF.get(),
        TFCMBlocks.PLACEHOLDER_WOODS.get(Wood.BlockType.BOOKSHELF).stream());
    extendBlockEntityType(
        TFCBlockEntities.SIGN.get(),
        TFCMBlocks.PLACEHOLDER_WOODS.get(Wood.BlockType.SIGN).stream());
    extendBlockEntityType(
        TFCBlockEntities.SIGN.get(),
        TFCMBlocks.PLACEHOLDER_WOODS.get(Wood.BlockType.WALL_SIGN).stream());
  }
